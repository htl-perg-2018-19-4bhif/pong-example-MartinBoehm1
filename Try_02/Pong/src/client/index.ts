...
let paddleOffWall:number;
let paddleHighest:number;
let paddleLowest:number;
window.addEventListener('load', async () => {
    const paddle = <HTMLDivElement>document.getElementsByClassName('paddle')[0];
    const paddleLeft = document.getElementById('left');
    paddleLeft.style.setProperty("left",`${50}px`);
    const paddleRight = document.getElementById('right');
    
    paddleRight.style.setProperty("right",`${document.documentElement.clientWidth-70}px`);
    ...
    function startMoving() {
      ...
      interval2 = setInterval(() => paddleLowest=currentPaddlePosition, 4);
      interval3 = setInterval(() => paddleHighest=currentPaddlePosition+100, 4);
    }
    function stopMoving() {
  ...
  function animateBall(currentBallPosition: Point, targetBallPosition: Point): Promise<{touchPosition: Point, touchDirection: Direction}> {
    ...
    return new Promise<{touchPosition: Point, touchDirection: Direction}>(res => {
      ...
      const interval = setInterval(() => {
        ...
        if ((animatedPosition.x - ballHalfSize.width) < 0) { animatedPosition.x=clientHalfSize.width; animatedPosition.y=clientHalfSize.height;}
        if (((animatedPosition.x - ballHalfSize.width) < paddleOffWall)&&((animatedPosition.y + ballHalfSize.width) > paddleLowest)&&((animatedPosition.y - ballHalfSize.width) < paddleHighest)) { touchDirection = Direction.left; }
        ...
        if ((animatedPosition.x + ballHalfSize.width) > clientSize.width) { animatedPosition.x=clientHalfSize.width; animatedPosition.y=clientHalfSize.height;}
        if (((animatedPosition.x + ballHalfSize.width) > clientSize.width-paddleOffWall)&&((animatedPosition.y + ballHalfSize.width) > paddleLowest)&&((animatedPosition.y - ballHalfSize.width) < paddleHighest)) { touchDirection = Direction.right; }
